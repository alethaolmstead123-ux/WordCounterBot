...
# Catch-all handler for all messages (debug)
@bot.message_handler(func=lambda message: True)
def debug_all(message):
    print(f"📨 Received: {message.text}")
    
    # If it's a command, let other handlers handle it
    if message.text and message.text.startswith('/'):
        return
    
    # Auto-analyze plain text
    if message.text:
        text = message.text
        ...
        response += f"Send /full_analysis for more details!"
...
Made with ❤️
"""
    # REMOVED parse_mode='Markdown' - THIS WAS THE PROBLEM!
    bot.reply_to(message, welcome_text)
    print(f"✅ Welcome sent")

@bot.message_handler(commands=['count'])
def quick_count(message):
    print(f"📨 /count from {message.from_user.username}")
...
def word_count_command(message):
    print(f"📨 /word_count from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        bot.reply_to(message, f"📝 Word Count: {count_words(text)}")
        print(f"✅ Word count sent")
    else:
        bot.reply_to(message, "❌ Please send text after the command.\nExample: /word_count Hello world")
        print(f"❌ No text provided")
...
def char_count_command(message):
    print(f"📨 /char_count from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        chars = count_characters(text)
        chars_no_space = count_characters_no_spaces(text)
        ...
        bot.reply_to(message, response)
        print(f"✅ Char count sent")
    else:
        bot.reply_to(message, "❌ Please send text after the command.\nExample: /char_count Hello world")
        print(f"❌ No text provided")

@bot.message_handler(commands=['sentence_count'])
def sentence_count_command(message):
    print(f"📨 /sentence_count from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        bot.reply_to(message, f"📖 Sentence Count: {count_sentences(text)}")
        print(f"✅ Sentence count sent")
    else:
        bot.reply_to(message, "❌ Please send text after the command.\nExample: /sentence_count Hello world. How are you?")
        print(f"❌ No text provided")

@bot.message_handler(commands=['paragraph_count'])
def paragraph_count_command(message):
    print(f"📨 /paragraph_count from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        bot.reply_to(message, f"📄 Paragraph Count: {count_paragraphs(text)}")
        print(f"✅ Paragraph count sent")
    else:
...
def syllable_count_command(message):
    print(f"📨 /syllable_count from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        bot.reply_to(message, f"🔊 Syllable Count: {count_syllables(text)}")
        print(f"✅ Syllable count sent")
    else:
        bot.reply_to(message, "❌ Please send text after the command.\nExample: /syllable_count Hello")
        print(f"❌ No text provided")

@bot.message_handler(commands=['unique_words'])
def unique_words_command(message):
    print(f"📨 /unique_words from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        ...
        bot.reply_to(message, response)
        print(f"✅ Unique words sent")
    else:
        bot.reply_to(message, "❌ Please send text after the command.\nExample: /unique_words Hello world hello")
        print(f"❌ No text provided")

@bot.message_handler(commands=['repeated_words'])
def repeated_words_command(message):
    print(f"📨 /repeated_words from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        ...
        if repeated:
            ...
            bot.reply_to(message, response)
            print(f"✅ Repeated words sent")
        else:
            bot.reply_to(message, "✅ No repeated words found.")
            print(f"✅ No repeats")
    else:
        bot.reply_to(message, "❌ Please send text after the command.\nExample: /repeated_words Hello world hello")
        print(f"❌ No text provided")

@bot.message_handler(commands=['longest_words'])
def longest_words_command(message):
    print(f"📨 /longest_words from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        ...
        if longest:
            ...
            for word in longest:
                response += f"- {word} ({len(word)} letters)\n"
            bot.reply_to(message, response)
            print(f"✅ Longest words sent")
        else:
            bot.reply_to(message, "No words found.")
            print(f"✅ No words")
    else:
        bot.reply_to(message, "❌ Please send text after the command.\nExample: /longest_words This is a sample text")
        print(f"❌ No text provided")

@bot.message_handler(commands=['shortest_words'])
def shortest_words_command(message):
    print(f"📨 /shortest_words from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        ...
        if shortest:
            ...
            for word in shortest:
                response += f"- {word} ({len(word)} letters)\n"
            bot.reply_to(message, response)
            print(f"✅ Shortest words sent")
        else:
            bot.reply_to(message, "No words found.")
            print(f"✅ No words")
    else:
        bot.reply_to(message, "❌ Please send text after the command.\nExample: /shortest_words This is a sample text")
        print(f"❌ No text provided")

@bot.message_handler(commands=['average_length'])
def average_length_command(message):
    print(f"📨 /average_length from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        avg = average_word_length(text)
        bot.reply_to(message, f"📊 Average Word Length: {avg} characters")
        print(f"✅ Average length sent")
    else:
        bot.reply_to(message, "❌ Please send text after the command.\nExample: /average_length This is a sample text")
        print(f"❌ No text provided")

@bot.message_handler(commands=['readability'])
def readability_command(message):
    print(f"📨 /readability from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        ...
        bot.reply_to(message, response)
        print(f"✅ Readability sent")
    else:
        bot.reply_to(message, "❌ Please send text after the command.\nExample: /readability This is a sample text")
        print(f"❌ No text provided")

@bot.message_handler(commands=['word_frequency'])
def word_frequency_command(message):
    print(f"📨 /word_frequency from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        ...
        for word, count in top_words:
            response += f"- {word}: {count} times\n"
        bot.reply_to(message, response)
        print(f"✅ Word frequency sent")
    else:
        bot.reply_to(message, "❌ Please send text after the command.\nExample: /word_frequency This is a sample text")
        print(f"❌ No text provided")

@bot.message_handler(commands=['full_analysis'])
def full_analysis_command(message):
    print(f"📨 /full_analysis from {message.from_user.username}")
    text = get_text_from_message(message)
    if text:
        ...
        bot.reply_to(message, response)
        print(f"✅ Full analysis sent")
    else:
        bot.reply_to(message, "❌ Please send text after the command.\nExample: /full_analysis Your text here")
        print(f"❌ No text provided")
